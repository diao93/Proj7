...
def get_data_voisins(client_id: int):
    """ 
    Calculate the nearest neighbors for a client and return their data.
    :param: client_id (int)
    :return: dataframe of similar clients (json).
    """
    if client_id not in data_scaled['SK_ID_CURR'].values:
        raise HTTPException(status_code=404, detail="Client ID not found.")
...
def shap_values_local(client_id: int):
    """ 
    Calculate the SHAP values for a specific client.
    :param: client_id (int)
